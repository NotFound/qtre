...
void showenterinfo (const char * funcname,
	const char * filename, size_t linenum)
{
	if (pout)
	{
		showinfo ("Enter", funcname);
		* pout << " in " << filename << " at " << linenum << endl;
	}
}

void showexitinfo (const char * funcname)
{
	if (pout)
	{
...
TraceFunc::TraceFunc (const char * filename, size_t linenum,
		const char * funcname_n, const char * shortname_n) :
	funcname (funcname_n ? funcname_n : "[unknown]"),
	...
	next (NULL)
{
	...
	* lastpos= this;
	...
	//showinfo ("Enter", funcname, filename, linenum);
	showenterinfo (funcname, filename, linenum);
	++indent;
}
...
TraceFunc::~TraceFunc ()
{
	...
	//showinfo ("Exit", funcname, filename, linenum);
	showexitinfo (funcname);
